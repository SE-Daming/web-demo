<template>
  <div>
    <el-table :data="orders" stripe style="width: 100%">
      ...
      <el-table-column prop="engName" label="分配的工程师" width="100"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="320">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.orderStatus==='0'" @click="cancel(scope.row)" type="text" size="small">取消</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    cancel(row) {
      this.$confirm('确定要取消吗？', '提示', {
        ...
        this.$http.post(`/orders/cancel/${row.id}`)
            .then(
                response => {
                  this.$message({
                    message: '取消成功',
        ...
        // 在这里可以继续执行取消操作，例如调用取消的 API 等
      }).catch(() => {
        // 用户点击了取消按钮的处理逻辑
        console.log('用户点击了取消按钮');
        // 在这里可以不做任何操作，或者根据需要处理其他逻辑
      });
    }
